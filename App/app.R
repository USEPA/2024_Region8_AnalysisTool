...
# Load the data
load("DataInput/InputData18.RData")
load("DataInput/SANDS_input14.RData")
                ...
                column(
                  ...
                  sidebarPanel(
                    ...
                    awesomeCheckboxGroup(inputId = "par_group_se", label = "Choose the parameter group",
                                         choices = c("Metal", "Nutrient", "Pestcide", "Other", "All"),
                                         inline = TRUE,
                                         status = "primary"),
                    uiOutput("multi_label"),
                    multiInput(inputId = "par_se", label = "Select the parameter to download",
                               choiceNames = Parameter_table$Group_Name,
                               choiceValues = Parameter_table$Standard_Name),
...
server <- function(input, output, session){
  ...
  observe({
    if ("All" %in% input$par_group_se){
      variable <- Parameter_table$Standard_Name
    } else {
      group_par <- Parameter_table %>%
